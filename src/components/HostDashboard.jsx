...
function RoleConfigPanel({
  ...
  return (
    <div className="bg-[#0b0e14] border border-[#272935] rounded-xl p-4 text-left space-y-3">
      ...
      {roomMode === 'preset' && (
        <div className="space-y-2">
          <select
            ...
            className="w-full bg-[#0c0f17] border border-[#272935] rounded px-2 py-1.5 text-sm text-gray-200 focus:outline-none focus:ring-1 focus:ring-[#8e44ad]"
          >
            {GAME_PRESETS.map(p => (
      ...
      {roomMode === 'custom' && (
        <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1">
          {Object.entries(ROLES).map(([roleId, role]) => (
            <div
              key={roleId}
              className="flex items-center justify-between bg-[#121622] border border-[#1f2430] p-2.5 rounded-lg"
            >
              <div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-6 h-6 rounded-full bg-[#1b2132] hover:bg-gray-800 flex items-center justify-center text-white"
                >
...
export default function HostDashboard({ onBackToRoleSelect }) {
  ...
  const handleResetSetup = async () => {
    if (roomCode) await supabase.from('rooms').delete().eq('id', roomCode);
    setRoomCode('');
    setPlayersList([]);
    setLastWinner(null);
    setStep('setup');
        ...
        <div className="glass-panel animate-fade-in space-y-6 max-w-md mx-auto w-full text-center">
          ...
          <div className="grid grid-cols-1 gap-4 pt-2">
            <button
              ...
              disabled={isLoading}
              className="group border border-[#272935] hover:border-[#8e44ad] bg-[#0c0f17] hover:bg-[#120f1c] p-4 rounded-xl text-left transition-all flex items-start gap-3.5"
            >
            ...
            <button
              ...
              disabled={isLoading}
              className="group border border-[#272935] hover:border-[#e67e22] bg-[#0c0f17] hover:bg-[#1c140f] p-4 rounded-xl text-left transition-all flex items-start gap-3.5"
            >
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
            ...
            <div className="glass-panel text-center flex flex-col space-y-4 md:col-span-1">
              ...
              <div className="w-40 h-40 mx-auto border-2 border-[#8e44ad] rounded-xl p-2 bg-[#0d0f14] flex items-center justify-center shadow-[0_0_20px_var(--color-primary-glow)]">
                <img
            ...
            <div className="glass-panel flex flex-col justify-between md:col-span-2 space-y-4">
              <div>
                ...
                <div className="mb-4 space-y-2">
                  ...
                  <div className="w-full h-2 bg-[#1b2132] rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-700"
                      style={{
                        width: `${progressPercent}%`,
                        background: isExact ? 'linear-gradient(90deg,#27ae60,#2ecc71)' : joined > totalCards ? 'linear-gradient(90deg,#c0392b,#e74c3c)' : 'linear-gradient(90deg,#8e44ad,#9b59b6)'
                      }}
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[300px] overflow-y-auto pr-2">
                  {playersList.map((player, idx) => (
                    <div key={player.id} className="flex items-center gap-3 bg-[#11141e] border border-[#202534] p-3 rounded-lg">
                      <div className="w-8 h-8 rounded-full bg-[#1b2132] border border-[#8e44ad] border-opacity-35 flex items-center justify-center font-bold text-xs text-[#8e44ad]">
                        {idx + 1}
                      </div>
                  ...
                  {playersList.length === 0 && (
                    <div className="col-span-2 text-center py-10 text-xs text-gray-500">
                      Đang đợi người chơi quét QR vào phòng...
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              {viewMode === 'wizard' && gameRoom.status === 'night_phase' && (
                <div className="glass-panel animate-fade-in space-y-6">
                  {nightWizardSteps.length > 0 ? (
                    <>
                      <div className="border-b border-[#272935] pb-4 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                          <span className="text-2xl">🌙</span>
                          <div>
                            <span className="text-[10px] uppercase tracking-widest text-[#f1c40f]">Bước {currentNightStep + 1} / {nightWizardSteps.length}</span>
                            <h3 className="text-xl font-bold font-serif text-white m-0" style={{ fontFamily: 'var(--font-gothic)' }}>
                              {nightWizardSteps[currentNightStep].name} thức giác...
                            </h3>
                          </div>
                        </div>
                      </div>

                      <div className="bg-[#121622] border border-dashed border-[#8e44ad] border-opacity-30 p-4 rounded-lg">
                        <span className="text-[10px] uppercase font-bold text-[#8e44ad] tracking-wider block mb-1">GỢI Ý LỜI THOẠI:</span>
                        <p className="text-xs italic text-gray-300 leading-relaxed m-0">
                      ...
                      <div className="space-y-4">
                            ...
                            {playersList.map(player => {
                              ...
                              return (
                                <button key={player.id}
                                  ...
                                  className={`p-3 rounded-lg text-xs font-semibold border transition-all text-left ${isSelected ? 'border-[#e74c3c] bg-[#e74c3c] bg-opacity-10 text-white' : 'border-[#272935] bg-[#0c0f17] text-gray-400 hover:border-gray-700'}`}>
                                  {player.name} {isSelected && '❤️'}
                                </button>
                              );
                            })}
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {playersList.filter(p => p.is_alive).map(player => (
                              <button key={player.id}
                                ...
                                className={`p-3 rounded-lg text-xs font-semibold border transition-all text-left ${nightSelections.protectedPlayer === player.id ? 'border-green-500 bg-green-500 bg-opacity-10 text-white' : 'border-[#272935] bg-[#0c0f17] text-gray-400 hover:border-gray-700'}`}>
                                🛡️ {player.name}
                              </button>
                            ))}
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {playersList.filter(p => p.is_alive).map(player => (
                              <button key={player.id}
                                ...
                                className={`p-3 rounded-lg text-xs font-semibold border transition-all text-left ${nightSelections.werewolfTarget === player.id ? 'border-[#c0392b] bg-[#c0392b] bg-opacity-10 text-white' : 'border-[#272935] bg-[#0c0f17] text-gray-400 hover:border-gray-700'}`}>
                                🐺 {player.name}
                              </button>
                            ))}
                          </div>
                        ...
                        {nightWizardSteps[currentNightStep].id === 'cursed_wolf' && (
                          <div className="space-y-4">
                            ...
                            <div className="bg-[#121622] p-4 rounded-lg border border-[#272935] space-y-3">
                              <div className="text-xs font-bold text-gray-300">
                                MỤC TIÊU ĐÀN SÓI CẮN: <span className="text-[#c0392b] font-bold ml-1">
                                  {playersList.find(p => p.id === nightSelections.werewolfTarget)?.name || 'Chưa chọn nạn nhân'}
                                </span>
                              </div>
                              <div className="flex items-center gap-4 pt-2 border-t border-[#1b2132]">
                                ...
                                <button
                                  ...
                                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all border ${nightSelections.curseActivatedTonight ? 'bg-red-600 border-red-600 text-white shadow-[0_0_10px_rgba(231,76,60,0.5)]' : 'border-[#272935] text-gray-400 disabled:opacity-20'}`}>
                                  {nightSelections.curseActivatedTonight ? '🔮 ĐANG NGUYỀN RỦ' : '🔮 Kích hoạt Nguyền'}
                                </button>
                              </div>
                            </div>

                            <span className="text-xs font-semibold text-gray-400 block uppercase tracking-wider">Hỗ trợ chọn nạn nhân cùng Đàn Sói (Nếu cần):</span>
                            <div className="grid grid-cols-2 gap-3">
                              {playersList.filter(p => p.is_alive).map(player => (
                                <button key={player.id}
                                  onClick={() => setNightSelections(prev => ({ ...prev, werewolfTarget: player.id }))}
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {playersList.filter(p => p.is_alive).map(player => (
                              <button key={player.id}
                                ...
                                className={`p-3 rounded-lg text-xs font-semibold border transition-all text-left ${nightSelections.serialKillerTarget === player.id ? 'border-red-600 bg-red-600 bg-opacity-10 text-white' : 'border-[#272935] bg-[#0c0f17] text-gray-400 hover:border-gray-700'}`}>
                                🔪 {player.name}
                              </button>
                            ))}
                          </div>
                        ...
                        {nightWizardSteps[currentNightStep].id === 'witch' && (
                          <div className="space-y-4">
                            <div className="flex items-center gap-4 bg-[#121622] p-4 rounded-lg border border-[#272935]">
                              <div className="text-xs font-bold text-gray-300 flex-1">
                                SÓI CẮN: <span className="text-[#c0392b] font-bold ml-1">
                              ...
                              <button
                                ...
                                className={`px-4 py-2 rounded-full text-xs font-bold transition-all border ${nightSelections.witchHealUsed ? 'bg-green-600 border-green-600 text-white' : 'border-[#272935] text-gray-400 disabled:opacity-20'}`}>
                                {nightSelections.curseActivatedTonight ? '🔮 Đã bị nguyền rủa' : witchPotions.hasHeal ? '🧪 Dùng bình Cứu' : '❌ Hết bình Cứu'}
                              </button>
                            </div>
                            <div className="space-y-2">
                              <span className="text-xs font-semibold text-gray-400 block">BÌNH ĐỘC (TÙY CHỌN):</span>
                              <div className="grid grid-cols-2 gap-3">
                                {playersList.filter(p => p.is_alive).map(player => (
                                  <button key={player.id}
                                    ...
                                    className={`p-3 rounded-lg text-xs font-semibold border transition-all text-left disabled:opacity-20 ${nightSelections.witchKillTarget === player.id ? 'border-purple-600 bg-purple-600 bg-opacity-10 text-white' : 'border-[#272935] bg-[#0c0f17] text-gray-400 hover:border-gray-700'}`}>
                                    ☠️ {player.name}
                                  </button>
                                ))}
                              </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              {playersList.filter(p => p.is_alive).map(player => (
                                <button key={player.id}
                                  ...
                                  className={`p-3 rounded-lg text-xs font-semibold border transition-all text-left ${nightSelections.seerTarget === player.id ? 'border-[#8e44ad] bg-[#8e44ad] bg-opacity-10 text-white' : 'border-[#272935] bg-[#0c0f17] text-gray-400 hover:border-gray-700'}`}>
                                  👁️ {player.name}
                                </button>
                              ))}
                            </div>
                            {nightSelections.seerTarget && (
                              <div className="bg-[#121622] border border-[#8e44ad] border-opacity-35 p-4 rounded-lg text-center animate-pulse">
                                <span className="text-xs text-gray-400">Kết quả soi bí mật:</span>
                                <div className="text-lg font-bold text-white mt-1">
                                  ...
                                  {['werewolf', 'wolf_cub', 'cursed_wolf'].includes(playersList.find(p => p.id === nightSelections.seerTarget)?.role)
                                    ? <span className="text-[#e74c3c]">MA SÓI 😈</span>
                                    : <span className="text-green-500">DÂN LÀNG/CHỨC NĂNG 😇</span>
                                  }
                                </div>
                              </div>
                            )}
                      ...
                      <button onClick={handleWizardNext} className="w-full btn-premium flex justify-center py-3 rounded-lg">
              ...
              {viewMode === 'reference' && (
                <div className="glass-panel animate-fade-in space-y-4">
                  ...
                  <p className="text-xs text-gray-400">Bấm 🟢/🔴 để cập nhật trạng thái Sống/Chết sau khi biểu quyết treo cổ.</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {playersList.map(player => (
                      <div key={player.id}
                        className={`border p-4 rounded-xl flex justify-between items-center transition-all ${player.is_alive ? 'border-[#272935] bg-[#0c0f17]' : 'border-red-950 bg-red-950 bg-opacity-10 opacity-50'}`}>
                        <div className="text-left">
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <div className="glass-panel space-y-4">
                <h4 className="font-serif text-sm font-bold text-[#8e44ad] tracking-wider m-0 flex items-center gap-1.5" style={{ fontFamily: 'var(--font-gothic)' }}>
                  <History className="w-4 h-4" /> NHẬT KÝ
                </h4>
                <div className="max-h-[220px] overflow-y-auto divide-y divide-[#272935] pr-1 space-y-3">
                  {gameLogs.map((log, index) => (
                    <div key={index} className="pt-2 text-[11px] text-gray-400 leading-relaxed">{log}</div>
                  ))}
                  {gameLogs.length === 0 && <div className="text-center text-xs text-gray-500 py-6">Chưa có nhật ký...</div>}
                </div>
              </div>
            </div>
          </div>
